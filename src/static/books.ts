...
async function fetchBooks(params: GetBooksParams) {
  const { searchText, sorting, startIndex } = params;

  const url = `https://www.googleapis.com/books/v1/volumes?key=AIzaSyDgZldmbQ9gaBPrIH4BW2-83lnFwprfry4&&startIndex=${startIndex * maxResults}&q=${searchText}&orderBy=${sorting}&maxResults=${maxResults}`
